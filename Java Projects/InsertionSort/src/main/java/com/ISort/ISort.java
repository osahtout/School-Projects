...
public class ISort {
	...
	if(args.length == 0)
	{
        ...
        start = System.nanoTime();		//stores right before the algorithm starts
		 done1 = insertionSort(tho);
		 end = System.nanoTime();		//stores right after the algorithm ends
...
try {	
		...
		if (args[0].toLowerCase().equals("debug"))
		{
			...
			start = System.nanoTime();		//stores the start of time right before
			done = ISortDebug(toSort);
			 end = System.nanoTime();		//stores in the end of time for the algorithm
